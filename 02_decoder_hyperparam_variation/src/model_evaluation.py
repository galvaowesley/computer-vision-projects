...
def evaluate_models_performance(checkpoint_dir, data_root, experiments, device="cuda"):
    ...
    for exp, model in tqdm(
        zip(experiments, models), total=len(experiments), desc="Avaliando modelos"
    ):
        model_name = exp["name"]
        ...
        with torch.no_grad():
            for img, target in dl_test:
                ...
                iou_scores.append(
                    iou_value.item() if hasattr(iou_value, "item") else float(iou_value)
                )
        end_time = time.time()
...
def plot_model_segmentations(
    ...
    fig, axs = plt.subplots(
        n_imgs, 2 + n_models, figsize=(4 * (2 + n_models), 4 * n_imgs)
    )
...
def plot_image_and_gt_grid(
    data_root, indices, split="test", save_pdf=False, pdf_path="image_gt_grid.pdf"
):
    """
